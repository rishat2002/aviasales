...
const Filter = ({select , selectCheap, selectFaster, selectOptimal,taskListLengthInitialState}) => {
  ...
  return (
    <div className='content__filter'>
     <button className={cheapClass} onClick={selectCheap} type='button'>Самый дешевый</button>
     <button className={fasterClass} onClick={selectFaster} type='button'>Самый быстрый</button>
     <button className={optimalClass} onClick={selectOptimal} type='button'>Оптимальный</button>
    </div>
  );
}
