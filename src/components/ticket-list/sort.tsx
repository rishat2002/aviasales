...
export const transferSort = (
  list: Array<componentTicketType>,
...
export const filterSort = (
  ...
  if (filterSortObj === 'optimal') {
    const averagePrice =
      list.reduce((acc, current) => (acc += current.price), 0) / list.length
    const averageDuration =
      list.reduce(
        (acc, current) => (acc = acc + current.segments[0].duration),
        0,
      ) / list.length
