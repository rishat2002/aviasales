...
const TransferFilter = ({checkboxs,selectOneTransfer,selectTwoTransfer
  ,selectThreeTransfer,selectAllTransfer,selectNoTransfer,taskListLengthInitialState}) => {
  useEffect(() => taskListLengthInitialState())
    /* eslint-disable */
  return (
    <div className='content__transfer-filter'>
      <span className='content__transfer-filter-title'>КОЛИЧЕСТВО ПЕРЕСАДОК</span>
      <label className='content__transfer-filter-label'>Все
        <Checkbox select={selectAllTransfer} checked={checkboxs.allTransfer}/>
        <span className='content__custom-checkbox'></span>
        </label>
      <label className='content__transfer-filter-label'>Без пересадок
        <Checkbox select={selectNoTransfer} checked={checkboxs.noTransfer}/>
        <span className='content__custom-checkbox'></span>
        </label>
      <label className='content__transfer-filter-label'>1 пересадка
        <Checkbox select={selectOneTransfer} checked={checkboxs.oneTransfer}/>
        <span className='content__custom-checkbox'></span>
        </label>
      <label className='content__transfer-filter-label'>2 пересадки
        <Checkbox select={selectTwoTransfer} checked={checkboxs.twoTransfer}/>
        <span className='content__custom-checkbox'></span>
        </label>
      <label className='content__transfer-filter-label'>3 пересадки
